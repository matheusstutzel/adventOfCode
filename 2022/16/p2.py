...
def dfs(v, dist, rates, time, p2= False, p = []):
    if(printa):
        print('looking',v,p2, p)
    p = p+[v]
    ...
    for n,d in dist[v].items():
        if d>time-2 or n in p:
            continue
        nt = time-d-1
        tot = max(tot, dfs(n,dist, rates, nt, p2, p)+rates[n]*(nt))
    return tot + (dfs("AA", dist,rates,26,False, p) if p2 else 0)
...
printa = True
print(dfs("AA",dist, rates, 26, True))
